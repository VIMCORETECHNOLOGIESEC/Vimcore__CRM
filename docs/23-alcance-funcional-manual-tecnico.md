...
**Fix de QA manual (A2, commit `e0cb7f8`, pusheado a `origin/test/gpt`)**: dentro de "Ver en vivo" (`esVistaSoloLectura` de `useVistaEmpresa`, ver ítem 29), "Acciones masivas" y la selección de filas quedan deshabilitadas en `LeadsPage.tsx`/`LeadsTable.tsx` — reasignar en lote es una acción de escritura, no soportada todavía para holding-wide viendo una empresa ajena. La columna de responsable sigue visible (no es una acción de escritura). Mismo criterio aplicado en el detalle del lead (`AccionesResponsable.tsx`, gateado desde `LeadDetallePage.tsx`, no desde el propio componente para no exigirle Router a sus tests).
...
**Fix de QA manual (A2, commit `e0cb7f8`, pusheado a `origin/test/gpt`)**: dentro de "Ver en vivo", el catálogo de productos ya no se autogateaba a nada más que `ADMINISTRADOR` — quedaba visible/editable también para un holding-wide en modo de solo lectura. `OportunidadesPage.tsx` ahora exige además `!esVistaSoloLectura` para mostrar el botón "Gestionar productos". De paso (tarea C1, mismo commit), `ProductosAdminDialog.tsx` se refactorizó de componente autocontenido a componente controlado (`open`/`onOpenChange`) para poder reutilizarlo también desde `NuevaOportunidadButton.tsx`: cuando un Administrador intenta crear una oportunidad y el catálogo de productos está vacío, ahora se lo redirige a gestionar productos en vez de mostrarle un selector vacío. Roles no Administrador conservan el selector con "Sin producto" como única opción (el producto es opcional).
...
**Fix de QA manual (A2, commit `e0cb7f8`, pusheado a `origin/test/gpt`) — el "solo lectura" pasó de intención a implementación real**: hasta esta ronda, "de solo lectura" en la descripción de arriba era solo la intención — no existía en todo el frontend ningún concepto real de modo de solo lectura (confirmado por grep global: solo había comentarios describiendo la intención, nunca un gate efectivo). `Bridges`, `Oportunidades` y `Leads` permitían de hecho altas, bajas, reactivaciones, gestión de productos/canales y reasignación en lote estando "dentro" de la vista de una empresa. Se agregó `esVistaSoloLectura` a `useVistaEmpresa.ts` (`true` cuando `user.sessionScope === "holding"` y hay un `empresaVistaId` activo) y se lo consume en cada pantalla para ocultar/deshabilitar sus acciones de escritura — ver las notas de "Listado de bridges" (ítem 6), "Listado y detalle de Oportunidad" (ítem 17), "Listado de leads" (ítem 3) y "Detalle de lead" (ítem 4, `AccionesResponsable`) para el detalle pantalla por pantalla. **Estilo "hero" del botón de entrada**: el ícono "Ver en vivo" en `EmpresaDetallePage.tsx` (ítem 28) ahora se compone con un círculo `bg-destructive` detrás (mismo criterio visual que el avatar de `WhatsAppChat`, pero en rojo para distinguir este acceso), en vez del ícono suelto anterior. La salida a `/empresas/:empresaId` (`SalirVistaEmpresaButton.tsx`) ya estaba implementada de una ronda anterior — sin cambios de código en esta, solo se confirmó su cobertura de test existente.
