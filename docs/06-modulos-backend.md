...
| M7 — Citas | ✅ Completo | — |
| M8 — Notificaciones y tiempo real | ⚠️ Implementado con productor pendiente | ✅ CERRADO (Bloque A, 2026-08-26): productor `TOKEN_POR_EXPIRAR` implementado (compartido con M4); notificación `LEAD_DATO_INCOMPLETO` a supervisores activos. ✅ CERRADO (2026-09-01): presencia online/offline in-memory sobre SSE (`usuario.presencia-cambiada`) y snapshot en `GET /usuarios`; Redis queda diferido hasta multi-réplica. |
...
| P2 | M4 | LinkedIn y X siguen pendientes. | Cobertura de captación incompleta. | ✅ CERRADO parcial (2026-08-31): receptor del webhook de LinkedIn (`GET`/`POST /api/v1/integraciones/linkedin/webhook`) implementado — handshake `challengeCode`/`challengeResponse`, verificación `X-LI-Signature` (`lib/firma-linkedin.ts`), encolado durable idempotente en `leads_recibidos` (sobre `LINKEDIN_PENDIENTE_DETALLE`), resolución de detalle vía `GET /leadFormResponses/{id}` y `adapters/linkedin.adapter.ts`. La UI/OAuth/discovery/suscripción de fuentes YA existían antes de este cierre y ya apuntaban a este endpoint sin que existiera — ahora un lead real entra al CRM. Mapeo preciso de `predefinedField` (nombre/teléfono/correo) vía la definición completa del formulario queda documentado como mejora futura, no bloqueante. X sigue pendiente. ✅ CERRADO (hotfix, 2026-08-31): `RED_SOCIAL_IMPLEMENTACION.LINKEDIN.implementado` pasó a `true` en `bridge.service.ts` — el flujo de punta a punta ya estaba construido y testeado, pero el catálogo (`listRedesSoportadas`/`GET /bridges/catalogo/redes-soportadas`) seguía excluyendo LINKEDIN. Tests actualizados: `tests/bridges.service.test.ts`, `tests/bridges.routes.test.ts`. X sigue pendiente (`implementado: false`, sin fuente real confirmada). |
