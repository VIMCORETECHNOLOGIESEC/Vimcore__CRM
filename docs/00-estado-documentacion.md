...
| `docs/blocks/{a..f}-*.md` | Mixto | Carve-out por bloque de la migración multi-tenant (D1-D14) | Bloques A y B: ✅ cerrados e implementados (ver su `Estado` individual en cada archivo). Bloque C: ⏳ en progreso — Etapa 1 (`c1807fe`) y Etapa 2 (`f45923e`) commiteadas y pusheadas a `origin/test/gpt`, Etapa 3 (RLS + suite adversarial) sin iniciar; ver `Estado` en `docs/blocks/c-aislamiento.md` para el detalle de handoff y un vacío detectado (riesgo #2, condición de carrera, sin tarea que lo implemente todavía). Bloques D-F: Borrador TO-BE — cada uno autocontenido (scope, D-refs, esquema/diseño movido, entrada/salida), no autorizan implementación por sí solos. |
