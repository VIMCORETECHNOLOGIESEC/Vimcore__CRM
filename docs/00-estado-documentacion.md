...
| `docs/22-integracion-linkedin-backend.md` | Borrador TO-BE | Diseño técnico candidato para LinkedIn Lead Sync | Detalla cambios backend por capa para OAuth, múltiples fuentes, webhook y reconciliación. No describe el AS-IS ni autoriza implementación fuera de una SDD change aprobada. |
| `docs/23-contrato-frontend-bridge-api.md` | Vigente | Contrato de endpoints para el frontend del bridge `API_EXTERNA` (bridgeApi) | Cubre creación, carga/corrección de conexión y mapeo, y prueba de conexión. El job de polling que trae los leads todavía no existe. |
