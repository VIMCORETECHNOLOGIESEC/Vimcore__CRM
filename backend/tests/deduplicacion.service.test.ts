 ...
 * se llama DIRECTO (sin HTTP) en todo este archivo — sin `TenantContext`,
 * cualquier lectura/escritura suya sobre `leads`/`lead_eventos` (RLS) ve 0
 * filas. `empresaId: null` (D3, unrestricted vía el rol de aplicación) en
 * vez de un id fijo de empresa: algunas pruebas de este archivo (bloque
 * "empresaId derivation + dedupe shadow scope") ejercitan deliberadamente
 * bridges de DOS empresas distintas en la misma prueba — un contexto fijo a
 * una sola empresa ocultaría la fila `bridges` de la otra vía RLS y rompería
 * ese escenario. Ninguna prueba de este archivo verifica aislamiento por
 * RLS en sí (eso lo cubren los 3 tests de aceptación de Group 0/D8 + la
 * suite adversarial de Group 6), así que el contexto unrestricted es seguro
 * acá.
 */
...
describe("deduplicacion.service — deduplicateLead", () => {
  it("1. crea cliente y lead nuevos cuando no existe cliente con ese teléfono", () =>
    conContexto(async () => {
  ...
  it("2. N=5 webhooks concurrentes con el mismo teléfono producen exactamente 1 cliente y 1 lead", () =>
    conContexto(async () => {
  ...
  it("3. D7: teléfono inválido + correo que coincide con cliente existente resuelve a ese cliente", () =>
    conContexto(async () => {
  ...
  it("4. teléfono inválido sin coincidencia de correo crea cliente nuevo con telefonoValido=false", () =>
    conContexto(async () => {
  ...
  it("5. reingreso — límite exacto: 89 días no reabre, 91 días crea REINGRESO en etapa NUEVO", () =>
    conContexto(async () => {
      const telefono = telefonoUnico();
  ...
  it("6. lead abierto existente registra interacción repetida y no crea lead nuevo aunque cambie otro dato", () =>
    conContexto(async () => {
      const telefono = telefonoUnico();
  ...
  it("7. D8: el nombre almacenado no se sobrescribe en un contacto repetido con nombre distinto", () =>
    conContexto(async () => {
      const telefono = telefonoUnico();
  ...
  it("D9: ambas ramas de interaccion_repetida marcan detalle.requiereNotificacion=true", () =>
    conContexto(async () => {
      // Rama lead_abierto.
  ...
  it("8. rollback forzado: cero cliente y cero lead_eventos huérfanos de ese intento (atomicidad de toda la transacción)", () =>
    conContexto(async () => {
      const telefono = telefonoUnico();
  ...
  it("M5 DD1: crea lead con redSocial/payloadOriginal/camposDinamicos poblados cuando la entrada los trae", () =>
    conContexto(async () => {
  ...
  it("M5 DD1: sin esos tres campos en la entrada (compatibilidad M3), el lead los deja NULL como antes", () =>
    conContexto(async () => {
      const entrada = entradaBase();
  ...
  it("Bloque C (D4, Fase 2/Stage 2 — cutover bloqueante): sin bridgeId, deduplicateLead RECHAZA crear el lead (ya NO degrada a empresaId null — retira 'compatibilidad M3' de WU4/D6)", () =>
    conContexto(async () => {
      const entrada: DeduplicacionInput = {
        ...entradaBase(),
        bridgeId: undefined,
...
describe("deduplicacion.service — Bloque B (Fase 3): empresaId derivation + dedupe shadow scope", () => {
  ...
  it("deriva Lead.empresaId desde el Bridge al crear el lead (dual-write, legado intacto)", () =>
    conContexto(async () => {
