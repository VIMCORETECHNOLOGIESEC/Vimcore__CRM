...
import { CrearAdministradorHoldingDialog } from "@/funcionalidades/usuarios/CrearAdministradorHoldingDialog";
...
describe("CrearAdministradorHoldingDialog", () => {
  ...
  it("muestra el título «Nuevo administrador del holding»", () => {
    renderDialog();

    expect(screen.getByText("Nuevo administrador del holding")).toBeInTheDocument();
  });
  ...
  it("con datos válidos, llama a onSubmit con `{ nombre, correo, password, rol: \"ADMINISTRADOR\" }` fijo", async () => {
    const user = userEvent.setup();
    const { onSubmit } = renderDialog();
