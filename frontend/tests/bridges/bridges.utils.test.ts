...
describe("evaluarAvisoBridge — token expirado (peor caso entre cuentas publicitarias)", () => {
  ...
  it("un bridge sin ninguna cuenta publicitaria no dispara ningún aviso de token", () => {
    ...
    const aviso = evaluarAvisoBridge(bridge, AHORA);
...
describe("evaluarAvisoBridge — token próximo a vencer (umbral de 7 días)", () => {
  it("marca tokenProximoAVencer cuando una cuenta VALIDO expira dentro de los 7 días", () => {
    ...
    expect(evaluarAvisoBridge(bridge, AHORA).tokenProximoAVencer).toBe(true);
  });
  ...
  it("no marca tokenProximoAVencer cuando la expiración está a más de 7 días", () => {
    ...
    expect(evaluarAvisoBridge(bridge, AHORA).tokenProximoAVencer).toBe(false);
  });
...
describe("tieneAvisoDestacado", () => {
  it("es true si cualquiera de las tres banderas es true", () => {
    expect(
      tieneAvisoDestacado({ tokenExpirado: true, tokenProximoAVencer: false, sinActividad: false }),
    ).toBe(true);
    expect(
      tieneAvisoDestacado({ tokenExpirado: false, tokenProximoAVencer: true, sinActividad: false }),
    ).toBe(true);
    expect(
      tieneAvisoDestacado({ tokenExpirado: false, tokenProximoAVencer: false, sinActividad: true }),
    ).toBe(true);
  });

  it("es false cuando ninguna bandera está activa", () => {
    expect(
      tieneAvisoDestacado({ tokenExpirado: false, tokenProximoAVencer: false, sinActividad: false }),
    ).toBe(false);
  });
});
...
describe("evaluarEstadoTokenCuenta — estado puntual por cuenta, sin agregación", () => {
  it("devuelve TOKEN_EXPIRADO para una cuenta con estadoToken TOKEN_EXPIRADO", () => {
    expect(evaluarEstadoTokenCuenta(cuentaFake({ estadoToken: "TOKEN_EXPIRADO" }), AHORA)).toBe(
      "TOKEN_EXPIRADO",
    );
  });

  it("devuelve ERROR_VERIFICACION para una cuenta con estadoToken ERROR", () => {
    expect(evaluarEstadoTokenCuenta(cuentaFake({ estadoToken: "ERROR" }), AHORA)).toBe(
      "ERROR_VERIFICACION",
    );
  });
  ...
  it("devuelve TOKEN_PROXIMO_A_VENCER para una cuenta VALIDO que expira dentro del umbral", () => {
    expect(
      evaluarEstadoTokenCuenta(cuentaFake({ estadoToken: "VALIDO", tokenExpiraEn: diasDesde(5) }), AHORA),
    ).toBe("TOKEN_PROXIMO_A_VENCER");
  });

  it("devuelve TOKEN_VALIDO para una cuenta VALIDO que expira lejos, o que no expira nunca", () => {
    expect(
      evaluarEstadoTokenCuenta(cuentaFake({ estadoToken: "VALIDO", tokenExpiraEn: diasDesde(60) }), AHORA),
    ).toBe("TOKEN_VALIDO");
    expect(
      evaluarEstadoTokenCuenta(cuentaFake({ estadoToken: "VALIDO", tokenExpiraEn: null }), AHORA),
    ).toBe("TOKEN_VALIDO");
  });
});
